...
def load_cifar(args):
    ...
    args.only_pl = False
    data_file = "{ds}_rho_{rho}_pr_{pr}_ltr_{imf}_only_pl_{only_pl}_sd{sd}.npy".format(
        rho=args.rho,
        ds=args.dataset,
        pr=args.partial_rate,
        imf=args.imb_ratio,
        only_pl=args.only_pl,
        sd=args.seed)

    save_path = os.path.join(args.data_dir, 'pre-processed-data', data_file)
    os.makedirs(args.data_dir, exist_ok=True)
    if not os.path.exists(save_path):
        data_dict = generate_data(args, save_path)
    else:
...
def generate_imb_pll_data(args):
    data, labels = get_train_data(args.dataset, '../data')
    ...
    if args.rho is not None:
        args.num_labels = int(len(train_labels) * args.rho)
        base_dir = os.path.dirname(os.path.dirname(__file__))
        lb_idx, ulb_idx = sample_labeled_unlabeled_data(args, train_labels, partialY, args.num_class,
                                                        args.num_labels, base_dir)

        if args.only_pl == 'True':
            train_data = train_data[lb_idx]
            train_labels = train_labels[lb_idx]
            partialY = partialY[lb_idx]
        else:
            partialY[ulb_idx] = torch.ones(args.num_class)
...
def generate_data(args, save_path):
