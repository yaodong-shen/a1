...
def sample_labeled_unlabeled_data(args, target, partialY, num_classes,
                                  lb_num_labels, base_dir, load_exist=True):

    pl_idx = (partialY.sum(dim=1) != num_classes).nonzero(as_tuple=True)[0]

    # 随机打乱
    shuffle_idx = torch.randperm(pl_idx.size(0))
    shuffled_pl_idx = pl_idx[shuffle_idx]

    lb_idx = shuffled_pl_idx[:lb_num_labels]

    # 取集合A-B
    all_idx = torch.arange(partialY.size(0))
    mask = ~torch.isin(all_idx, lb_idx)
    ulb_idx = all_idx[mask]

    return lb_idx, ulb_idx


def ensemble_fusion(pred_head, pred_tail):
    p1, p2 = pred_head, pred_tail
    p1_entropy, p2_entropy = torch.sum(-p1 * torch.log(p1 + 1e-5), dim=1, keepdim=True), torch.sum(
        -p2 * torch.log(p2 + 1e-5), dim=1, keepdim=True)
    w1, w2 = 1 / (p1_entropy + 1e-5), 1 / (p2_entropy + 1e-5)
    w2 = w2 / (w1 + w2)
    w1 = 1 - w2
