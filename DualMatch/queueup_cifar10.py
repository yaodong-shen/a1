...
def run(times, gpu):
    ...
    pr_list = [0.1, 0.3, 0.5, 0.7]
    imb_ratio_list = [10, 20]  # 添加imb_ratio可选值
    ...
    cmd = (
        ...
        f"--dataset cifar10 --epochs 800 --batch-size 256 --lr 0.01 --wd 1e-3 --t 2 --save_ckpt "
        f"--imb_ratio {imb_ratio} --rho 0.1 --partial_rate {pr} --seed {seed} >> output.log &"
    )
...
if __name__ == '__main__':
    narrow_setup(interval=70, m_need=7000, times=0, gpus=list(range(8)), max_tasks=2)